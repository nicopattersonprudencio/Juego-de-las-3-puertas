...
@onready var padre = get_parent().get_parent().get_parent()
...
const primera_linea: String = "Lees las escrituras arriba de la puerta"
const segunda_linea: String = "'Ambas puertas dicen la verdad'"
const tercera_linea: String = "¿Quieres abrir esta puerta?"

const quarta_linea: Array[String] = ["No sientes la necesidad de abrir esta puerta"]
...
func _unhandled_input(event):
	...
	if jugador_en_rango and Input.is_action_just_pressed("interactuar"):
		...
		if not GameVariables.nivel_2_alcanzado:
			var lines_to_show: Array[String] = []
		
			if not GameVariables.puerta_intro_mostrada and not GameVariables.nivel_2_alcanzado:
				lines_to_show.append(primera_linea)
				GameVariables.puerta_intro_mostrada = true
			
			lines_to_show.append(segunda_linea)
			lines_to_show.append(tercera_linea)
...
func _on_opcion_seleccionada(texto,index):
	...
	if index == 1 and unico1:
		...
		if GameVariables.nivel_2_alcanzado:
			DialogManager.start_dialog(Vector2(-130, 125), quarta_linea)

		else:
			TransitionScreen.fade_black()
			await TransitionScreen.on_transition_finished
			padre.reiniciar()
			jugador.global_position = Vector2(2, 103)
	unico1 = false
	pulsar_x = true
