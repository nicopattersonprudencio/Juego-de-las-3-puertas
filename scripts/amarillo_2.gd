...
func _unhandled_input(event):
	...
	if jugador_en_rango and Input.is_action_just_pressed("interactuar"):
		...
		if not GameVariables.nivel_3_alcanzado:
			...
			DialogManager.start_dialog(Vector2(-130, -115), lines)
		else:
			unico1 = true
	...
	if jugador_en_rango and Input.is_action_just_pressed("intercambiar") and pulsar_x and GameVariables.intercambiar:
		...
		DialogManager.start_dialog(Vector2(-130, -115), lines2)
...
func _on_opcion_seleccionada(texto,index):
	...
	if index == 1 and unico1:
		...
		if GameVariables.nivel_3_alcanzado:
			DialogManager.start_dialog(Vector2(-130, -115), lines1)
		else:
			var jugador = get_tree().get_first_node_in_group("jugador")
			TransitionScreen.transition()
			await TransitionScreen.on_transition_finished
			padre.reiniciar()
			jugador.global_position = Vector2(0, 55)
	unico1 = false
	pulsar_x = true
...
func _on_dialogo_terminado():
	if debe_mostrar_opciones:
		...
		unico1 = true
		OpcionManager.mostrar_opciones(opciones, Vector2(-130, -240))
