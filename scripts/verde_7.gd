...
const lines: Array[String] = [
	"'La puerta izquierda es la salida'",
	"'La puerta azul es la salida'",
...
func _on_opcion_seleccionada(texto,index):
	...
	if index == 1 and unico:
		if int(puerta.global_position.x) == 64 and int(puerta2.global_position.x) == 0 and int(puerta3.global_position.x) == -64:
